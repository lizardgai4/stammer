...
def apply_color_mode(ffmpeg_call, color_mode):
    ...
    if color_mode == "8fast":
        ...
        color_strs = [
            "-vf",
            "split[s0][s1];[s0]palettegen=256:0:stats_mode=single[p];[s1][p]paletteuse=new=1:dither=bayer",
        ]
...
class VideoHandler:
    def __init__(
        ...
        self.best_match_count = int(
            len(matcher.get_best_matches()) * matcher.frame_length / frame_length
        )
    ...
    def _calculate_frame_ranges(self):
        sorted_unique_frames = sorted(
            list(set([i.item() for i in self.matcher.best_matches]))
        )
    ...
    def create_output_proc(self):
        ...
        return subprocess.Popen(call, stdin=subprocess.PIPE, stdout=subprocess.DEVNULL)
...
class VideoHandlerMem(VideoHandler):
    def __init__(self, *args):
        ...
        self.frame_length_max = self.frame_length / max(
            self.frame_length, self.matcher.frame_length
        )
    ...
    def get_progress_strings(self):
        ...
        strs.append(
            f"{self.framecount-self.cache.decayed_items}/{self.framecount} cached frames"
        )
        return strs
