...
class AudioMatcher:
    ...
    def make_normalized_bands(self, frames_input):
        ...
        bands = np.divide(
            np.add.reduceat(spectra, split_points, axis=1), section_lengths
        )  # average value in each band
...
class BasicAudioMatcher(AudioMatcher):
    ...
    def find_matches(self):
        scores = np.dot(
            self.modulator_bands.astype(np.float32, copy=False),
    ...
    def build_output_audio(self):
        ...
        for i in range(len(self.modulator_frames)):
            ...
            output_audio[start:end] += self.get_rescaled_frame(
                carrier_frame, modulator_frame
            )
        return output_audio
...
class CombinedFrameAudioMatcher(AudioMatcher):
    ...
    def best_match(self, modulator_band):
        ...
        while (
            ...
            and (
                (not proj_indices)
                or len(proj_indices) == 1
                or len(proj_indices) != self.MAX_BASIS_WIDTH
            )
        ):
            dot_products = np.sum(self.carrier_bands * modulator_band, axis=1)
            ...
            delta = post - pre
        if (
            np.abs(proj_indices[-1]) < basis_epsilon
            or np.abs(coeffs[-1]) < basis_epsilon
        ):
            proj_indices.pop()
            coeffs.pop()
    ...
    def find_matches(self):
        ...
        self.best_matches = np.zeros(
            (len(self.modulator_bands), self.MAX_BASIS_WIDTH), np.int32
        ) - np.ones((len(self.modulator_bands), self.MAX_BASIS_WIDTH), np.int32)
    ...
    def build_output_audio(self):
        ...
        for i in range(len(self.modulator_frames)):
            composed_frame = self.get_carrier(
                self.best_matches[i], self.basis_coefficients[i]
            )
            if not composed_frame is None:
                output_audio[
                    i * self.samples_per_frame : i * self.samples_per_frame
...
class UniqueAudioMatcher(BasicAudioMatcher):
    def find_matches(self):
        if len(self.carrier_bands) < len(self.modulator_bands):
            logging.warning(
                f"Carrier is shorter than modulator ({len(self.carrier_bands)} frames vs {len(self.modulator_bands)} frames). Trimming modulator to the length of carrier"
            )
...
class WeightedAudioMatcher(BasicAudioMatcher):
    ...
    def find_matches(self):
        ...
        weighted_carriers = (
            self.carrier_bands.astype(np.float32) * w
        )  # (n_frames, n_bins)
